    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Your marketing overview</p>
        </div>
        <button
          onClick={fetchLeads}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-6 lg:mb-8">
        {statCards.map((stat, index) => (
          <div
            key={stat.name}
            className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 animate-fade-in-up"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {recentLeads.map((lead) => (
                <div key={lead.id} className="p-3 sm:p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      <span className="text-xl sm:text-2xl flex-shrink-0">{getSourceIcon(lead.source)}</span>
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 truncate text-sm sm:text-base">{lead.name}</p>
                        <p className="text-xs sm:text-sm text-gray-500 truncate">{lead.company || lead.email}</p>
                      </div>
                    </div>
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ${getStatusColor(lead.status)}`}>
                      {lead.status}
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
            ...
            >
              <div className="p-2 sm:p-3 bg-secondary text-primary rounded-lg flex-shrink-0">
                <Search className="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
              <div className="min-w-0">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Run Lead Scrapers</p>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Find new leads from Google Maps, Instagram</p>
              </div>
            </a>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Manage Leads</p>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Add, import, or edit your leads</p>
              </div>
            </a>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Email Campaigns</p>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Create and manage campaigns</p>
              </div>
            </a>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="font-medium text-gray-900 text-sm sm:text-base">Email Templates</p>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Customize your email templates</p>
              </div>
            </a>
          </div>
        </div>
      ...
      {/* Automation Status */}
      <div className="mt-6 lg:mt-8 bg-gradient-to-r from-primary to-accent rounded-xl p-4 sm:p-6 text-white">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2">System Status</h3>
            <p className="text-gray-300 text-sm sm:text-base">Your marketing automation is connected to Supabase.</p>
          </div>
          <div className="sm:text-right">
            <div className="flex items-center gap-2 mb-1 sm:mb-2">
              <span className="w-3 h-3 bg-green-400 rounded-full animate-pulse"></span>
              <span className="text-sm sm:text-base">Database Connected</span>
            </div>
            <p className="text-xs sm:text-sm text-gray-300">Emails paused (testing mode)</p>
          </div>
        </div>
      </div>
    </div>
